...
def AirbnbCABATransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        data = X
        data = _dropNotUsedColumns(data)
        data = _processBathRooms(data)
        data = _processBedRooms(data)
        data = _convertToBoolean(data)
        data = _processPrice(data)
        data = _processPropertyType(data)
        data = _processOutLiers(data)
        data = _processHostDaysActive(data)
        data = _processAmenities(data)
        data = _processVerifications(data)
                
        data = data.fillna(0)

        data = _processNeighbourhood(data)
        data = _dropTextColumns(data)
        data = _processGetdummies(data)
        data = _convertToInt(data)
    ...
    def _extractBathRoomType(row):  
        if ("shared" in row["bathrooms_text"].lower()):
            return "shared"
        return "private"

    def _extractBathQuantity(row):  
        f = re.findall('\d*\.?\d+',row["bathrooms_text"])
        if (len(f)>0):
            return f[0]
        return "1"
    
    def _processBathRooms(data):
        data["bathrooms_text"] = data["bathrooms_text"].astype(str)
        data["bathroomtype"] = data.apply(lambda x:_extractBathRoomType(x),axis =1)   
        data["bathrooms"] = data.apply(lambda x:_extractBathQuantity(x),axis =1)   
    ...
    def _extractBedRoom(row):
        try:
            resultado=None
            ambientes=0
            if (row['description'] and isinstance(row['description'],str)):
                resultado = _patron_regex.search(row['description'])
            if ((resultado is None) and row['name'] and isinstance(row['name'],str)):
                resultado = _patron_regex.search(row['name'])
            if (resultado is not None):
                qty = 1
                if (resultado.group("numero") in _numbers_es):
                    qty = _numbers_es.index(resultado.group("numero"))
                    if (qty == 0):
                        qty = 1
                elif(resultado.group("numero") in _numbers_en):
                    qty = _numbers_en.index(resultado.group("numero"))
                    if (qty == 0):
                        qty = 1
    ...
    def _processBedRooms(data):
        #Relleno con patron regex según la palabra ambiente
        data.loc[data.bedrooms.isnull(),'bedrooms'] = data.loc[data.bedrooms.isnull()].apply(_extractBedRoom,axis=1)
    ...
    def _convertToBoolean(data):
        data.replace({'f': 0, 't': 1}, inplace=True)
        return data
    
    def _processPrice(data):
        data.price = data.price.str[1:-3]
        data.price = data.price.str.replace(",", "")
        data.price = data.price.astype('int64')
        return data
    
    def _processPropertyType(data):
    ...
    def _findOutliers(df, column, limit=4):
        """
    ...
    def _processOutLiers(data):
        # quitar outliers de precios
        data = data[np.logical_not(_findOutliers(data, 'price',limit=8))]
        # quitar outliers de baños
        data = data[np.logical_not(_findOutliers(data, 'bathrooms',limit=6))]

        # quitamos outliers de camas
        data = data[np.logical_not(_findOutliers(data, 'beds',limit=6))]

        # quitamos outliers de noches minimas
        data = data[np.logical_not(_findOutliers(data, 'minimum_nights',limit=14))]

        return data
    
    def _processHostDaysActive(data):
    ...
    def _processVerifications(data):
    ...
    def _processNeighbourhood(data):
